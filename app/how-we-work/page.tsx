...
export default function HowWeWork() { return <><Header/><main id="main-content"><section className="page-hero"><p className="eyebrow">How we work</p><h1>Start with the work. Strengthen the system.</h1><p>Good Move works alongside leaders and teams to turn operating friction into practical improvement.</p></section><section className="steps">{steps.map(([n,title,copy])=><article key={n}><span>{n}</span><h2>{title}</h2><p>{copy}</p></article>)}</section><section className="offer"><div><p className="eyebrow">A focused place to begin</p><h2>Operations Clarity Sprint</h2></div><div><p>Our primary engagement focuses on one high-friction workflow. We clarify the current state, define a more capable future state, and give your team a prioritized implementation roadmap.</p><ul><li>Current-state view of how the work operates</li><li>Patterns creating friction or fragility</li><li>Future-state operating recommendations</li><li>Focused next moves your team can execute</li></ul><SiteLink className="button" href="/operations-clarity-sprint">Explore the Sprint <span>↗</span></SiteLink></div></section></main><Footer/></>; }
