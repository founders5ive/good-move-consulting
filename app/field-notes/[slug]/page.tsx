...
export default async function NotePage({params}:{params:Promise<{slug:string}>}) { const note=getFieldNote((await params).slug); if(!note)notFound(); return <><Header/><main><article className="note"><header><SiteLink href="/field-notes">← All Field Notes</SiteLink><p className="eyebrow">Field Note {note.number}</p><h1>{note.title}</h1><p>{note.excerpt}</p><time>{formatDate(note.date)}</time></header><div className="prose"><Markdown source={note.body}/></div></article><section className="note-cta"><h2>What workarounds has your organization learned to live with?</h2><SiteLink className="button" href="/contact">Start a conversation <span>↗</span></SiteLink></section></main><Footer/></>; }
