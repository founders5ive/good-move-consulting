...
export type FieldNote = { slug:string; title:string; number:string; date:string; excerpt:string; body:string; reflection?:string; readingMinutes:number };
...
export function getFieldNotes(): FieldNote[] {
  ...
  return fs.readdirSync(directory).filter((file)=>file.endsWith(".md")).map((file)=>{
    ...
    return { slug:file.replace(/\.md$/, ""), title:meta.title, number:meta.number, date:meta.date, excerpt:meta.excerpt, body:body.trim(), reflection:meta.reflection, readingMinutes:Math.max(1,Math.ceil(body.trim().split(/\s+/).length/200)) };
